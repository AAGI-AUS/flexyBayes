---
title: "Tutorial 05: Default priors and the `fb_prior()` DSL"
date: "2026-08-26"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Tutorial 05: Default priors and the fb_prior() DSL}
...
- **Boundary problems.** When the variance is near zero, the
  ...
  all) reflects this. A prior with a mode away from zero
...
Fixed-effect coefficients work the other way round from the variance
components. There is no flexyBayes default on them: unless you supply
`prior_fixed_sd`, each engine keeps its own. brms leaves the
population-level coefficients flat and puts a response-centred
`student_t` on the intercept, and INLA uses `prec = 0.001` on the slopes
with a flat intercept. `prior_summary()` says which of the two a fit
ran under, and `triangulate()` reads the same record, so a pair of fits
whose fixed effects were never given a shared prior is reported as not
compared rather than compared anyway.

The asymmetry is deliberate. A blanket $\mathcal{N}(0, 100)$ on the
intercept is not weak information for a response whose central tendency
is far from zero -- `sleepstudy` sits near 300 ms -- so applying it
unasked would replace brms's response-centred intercept prior with a
different model. Variance components have no such problem, which is why
they do carry a package default.

Supply `prior_fixed_sd = s` and every fixed effect -- the intercept,
factor-contrast levels, continuous slopes, factor-by-continuous
interactions, and `I()`-expression terms -- carries
$\mathcal{N}(0, s)$. It arrives as one `normal(0, s)` row per
fixed-effect class on brms, and as `control.fixed` with
`prec = 1 / s^2` on the slopes and the intercept alike on INLA. The
documented starting value is `s = 100`, and three things recommend it
when a single scale is wanted:
...
2. **Literature anchoring.** Gelman et al. (2008, *Annals of Applied
   ...
   no regularisation.
3. **Sensitivity bracketing.** On `sleepstudy`, halving
   `prior_fixed_sd` to 50 and doubling it to 200 both leave the
   posterior where it was. Below about 20 the prior starts to bite on
   coefficients of order $\beta \sim 10$, which is the intended
   behaviour when the analyst chooses a smaller value deliberately for
   shrinkage.

Set individual coefficients with `b("name") ~ normal(mean, sd)` in
`fb_prior()`, which overrides the scalar for the coefficients it names
and leaves the rest to whichever of the two routes above is in force.

# 3. The `fb_prior()` DSL

`fb_prior()` accepts variadic two-sided formulas of the form
`target ~ distribution(args)` and returns a structured prior object
that any `flexybayes()`, `fb_inla()`, or `fb_brms()` call can
consume.
...
2. A half-normal(2.5) -- a more concentrated weakly-informative prior,
   recommended alongside half-Cauchy for very small $J$ (Gelman,
   2006, §4).
...
> **`backend = "brms"` pinned throughout this section.** INLA's fit
> to this exact model (`sleepstudy`'s `Subject` random intercept) has
> a known instability -- the variance component can collapse to
> numerically zero, and which run collapses is not reproducible even
> at a fixed seed. Comparing *priors* needs a *reliable* engine
> underneath, and brms (Stan HMC) does not share the failure mode. The
> *getting started* and *hierarchical models* vignettes discuss the
> INLA instability itself.
...
fit_default <- flexybayes(
  ...
  backend = "brms",
  n_samples = 1000, warmup = 1500, chains = 2,
...
basis, the small-$J$ half-normal guidance, and the override knobs. We
keep this fit as the reference.


``` r
priors_hn <- fb_prior(
  sigma                 ~ half_normal(scale = 2.5),
  sd(group = "Subject") ~ half_normal(scale = 2.5)
)
fit_hn <- flexybayes(
  fixed = Reaction ~ Days, random = ~ Subject, data = sleepstudy,
  backend = "brms", prior = priors_hn,
  n_samples = 1000, warmup = 1500, chains = 2,
  verbose = FALSE, mcmc_verbose = FALSE
)
```
...
A posterior-mean comparison on the subject-level SD:


``` r
extract_sigma <- function(fit) {
  d <- flexyBayes::fb_as_draws_simple(fit)
  s <- d[["sd_Subject__Intercept"]]
  c(mean = mean(s), q025 = quantile(s, 0.025), q975 = quantile(s, 0.975))
}
rbind(default    = extract_sigma(fit_default),
      halfnormal = extract_sigma(fit_hn),
      legacy     = extract_sigma(fit_legacy))
#>                mean q025.2.5% q975.97.5%
#> default    39.46677  27.31404   57.31749
#> halfnormal 16.92567  14.46865   19.53223
#> legacy     34.44136  24.98739   46.96731
```

The uniform default and the legacy lognormal land close together
here. The half-normal(2.5) pulls the posterior mean noticeably lower
-- its thinner tails are more concentrated at this scale than either
the uniform or the lognormal(0, 1) implies, and with eighteen
subjects the data do not have enough leverage to pull free of that
concentration. None of the three is *wrong* -- they encode different
prior information. *That sensitivity is the diagnosis*: with eighteen subjects, the
...
![Histogram of the simulated mean(y) under the prior alone (200 draws from the uniform-default variance components and the fixed-effect priors), with the observed mean reaction time marked in firebrick -- the observed value should land inside the bulk of the histogram, not out in a tail.](flexyBayes-05-priors-figs/prior-predictive-1.png)
...
`fb_prior()` is the cross-engine *interlingua*: one prior
specification compiles to the parameterisation each backend expects.
Section 3's target and family tables describe the DSL, not either
engine. What each engine can carry is narrower, and it is different on
the two of them.

The table below is the translation contract. **T** means the engine
carries the pair and the prior arrives in the engine argument named
underneath. **R** means the fit refuses by name, saying which row of
this table it fell outside and naming both remedies -- re-express the
prior in a distribution that engine does carry, or move the fit to the
other backend. Nothing is dropped in silence: a specification that
cannot arrive stops the fit rather than leaving `prior_summary()`
describing a prior the sampler never saw.

INLA:

| Target | `normal` | `half_normal` | `half_cauchy` | `cauchy` | `student_t` | `exponential` | `gamma` | `lkj` | `pc` | `uniform` |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| `sigma` | R | T | R | R | R | T | R | R | T | T |
| `sd(group)` | R | T | R | R | R | T | R | R | T | T |
| `smooth(var)` | R | T | R | R | R | T | R | R | T | T |
| `b(name)` | T | R | R | R | R | R | R | R | R | R |
| `cor(group)` | R | R | R | R | R | R | R | R | R | R |

brms:

| Target | `normal` | `half_normal` | `half_cauchy` | `cauchy` | `student_t` | `exponential` | `gamma` | `lkj` | `pc` | `uniform` |
|---|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|:-:|
| `sigma` | T | T | T | T | T | T | T | R | T | T |
| `sd(group)` | T | T | T | T | T | T | T | R | T | T |
| `smooth(var)` | R | R | R | R | R | R | R | R | R | R |
| `b(name)` | T | R | R | R | T | R | R | R | R | R |
| `cor(group)` | R | R | R | R | R | R | R | R | R | R |

The arrival routes, one per translating pair:

| Engine | Target | Arrives through |
|---|---|---|
| INLA | `sigma`, `sd`, `smooth` | the term's `hyper` list, on $\theta = \log(\text{precision})$, and `control.family` for `sigma` |
| INLA | `b` | `control.fixed`, as a per-coefficient `mean` and `prec` |
| brms | `sigma`, `sd` | a brms prior row with the parameter's lower bound at zero |
| brms | `b` | a brms prior row |

Three entries are worth reading twice. On INLA, `exponential(rate)` *is*
the penalised-complexity prior rather than an approximation to it,
because `pc.prec(U, alpha)` states
$\Pr(\sigma > U) = \alpha$, which is
$\text{Exponential}(-\log(\alpha)/U)$ on the SD scale. On brms,
`half_normal` and `half_cauchy` are the zero-centred normal and Cauchy
that truncation at zero makes them, so they and `normal(0, s)` are the
same prior on a variance component. And `cor()` is a refusal on both
engines because no model this package emits carries a correlation
hyperparameter at all -- correlated random slopes are refused at ingest,
so the target has nothing to attach to rather than an engine gap.

The *cross-engine triangulation* vignette demonstrates the pairwise
comparison. Direct INLA dispatch on brms-shaped formulas is available
via `fb_inla()` (equivalently `flexybayes(..., backend = "inla")`), and
gate-driven routing via `backend = "auto"` is documented in the
*hierarchical models* and *understanding dispatch and refusals*
vignettes.
...
fit_pc <- flexybayes(
  fixed = Reaction ~ Days, random = ~ Subject, data = sleepstudy,
  backend = "brms", prior = priors_pc,
  n_samples = 1000, warmup = 1500, chains = 2,
  verbose = FALSE, mcmc_verbose = FALSE
)
extract_sigma(fit_pc)
#>       mean  q025.2.5% q975.97.5% 
#>   32.05827   23.64657   42.47482
```
...
directly. brms and Stan can express a horseshoe hierarchy natively,
so this is not blocked by the 0.9.3 engine withdrawal (see `NEWS.md`)
the way the genotype-by-environment structured-covariance family is (the
genotype-by-environment section of the *formula surface* vignette) -- it is not yet wired into
`fb_prior()` yet.

`fb_prior()` does not yet expose `horseshoe()` as a target. A future
release will add `horseshoe(global, slab)` for fixed-effect
coefficient vectors, routed through brms. Also planned for a
...
deviation* scale. INLA's internal parameterisation is precision, and
...
**An implausible variance-component estimate might be an engine
artefact, not a real posterior.** Section 4's `backend = "brms"` pin
is not incidental -- on some small-group Gaussian models, INLA's
inner optimiser can land on a numerically-degenerate near-zero
variance regardless of which prior is stated, and it is not
reproducible run to run. Before reading anything into a
suspiciously-extreme variance component from INLA, cross-check it
with `backend = "brms"`.

# 10. Active prompts

1. Refit the sleep-study model with `prior_vc_sd = 0.1`. How does
   the posterior on `sd_Subject__Intercept` compare with the uniform
   default?
...
3. Run a prior-predictive check on a Poisson hierarchical model
   (use `MASS::epil`). Compare the uniform default to a half-normal(2.5)

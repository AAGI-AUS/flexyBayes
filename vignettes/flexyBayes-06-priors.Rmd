...
3. **Consequence on `sleepstudy`.** On `Reaction ~ Days` (intercept
   ...
   and the prior shrinks neither parameter perceptibly.
4. **Sensitivity bracketing.** Halving `prior_fixed_sd` to 50
   produces an indistinguishable posterior on `sleepstudy`, and doubling
...
Override globally with `prior_fixed_sd = ...`, or individual
...
#> default    39.91549  27.13793   57.92398
#> halfnormal 16.97524  14.60043   19.76518
#> legacy     34.32197  24.53421   47.69688
```
...
expects. brms receives a direct Stan prior statement on the
standard-deviation scale, and INLA receives a `pc.prec` block tuned to
match the same upper-tail statement (only a subset of prior families
translate to each backend -- Section 3's family table applies
per-backend, not universally, and brms in particular does not yet
translate `half_cauchy` anywhere in the DSL -- `uniform`, `pc`, and
`half_normal` are the supported SD-scale families on that path, and
`normal` / `student_t` the supported fixed-effect families). The
*cross-engine triangulation*
vignette demonstrates the pairwise comparison. Direct INLA dispatch
on brms-shaped formulas is available via `fb_inla()` (equivalently
`flexybayes(..., backend = "inla")`), and gate-driven routing via
`backend = "auto"` is documented in the *hierarchical models* and
*understanding dispatch and refusals* vignettes.

...
#>   Residual : ~ar1(Days_f):Subject 
#>   Family : gaussian ( identity link )
#>   MCMC   : 2 chain(s) x 1000 samples (warmup = 2000 ) -- 6.6 sec
#>   Params : 4 monitored; 2 fixed, 1 random terms
#>   Representation: exact
#>   Engine:         greta MCMC
#>   Max Rhat: 35.571  [!] 
...
flexybayes(
  ...
  residual   = ~ ar1(row):ar1(col),                  # local spatial AR1
  data = ...
)
```
...
`flexyBayes` covers regular grids with AR1 correlation. It does
*not* cover:
...
- **continuous-time temporal correlation** — the
  Ornstein–Uhlenbeck and continuous-AR processes are on the
  roadmap;
...
1. Refit `stroup.nin` with `residual = ~ units` (no spatial structure).

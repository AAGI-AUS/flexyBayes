---
title: "7. After the fit: summaries, comparison, triangulation"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{7. After the fit: summaries, comparison, triangulation}
...
<dl>
<dt>The problem</dt>
<dd>The fit is done. Getting the numbers out, comparing two models, and
checking that the answer is a property of the data rather than of the
engine.</dd>
<dt>In ASReml</dt>
<dd><code>summary()</code>, <code>predict()</code>, <code>wald()</code>,
and a change in REML log-likelihood.</dd>
<dt>In flexyBayes</dt>
<dd><code>tidy()</code>, <code>predict()</code>, <code>loo()</code>, and
<code>triangulate()</code>.</dd>
<dt>What the posterior adds</dt>
<dd>Any function of the parameters is computed on the draws. And with
two engines, a check no single fit can perform on itself.</dd>
<dt>What it costs</dt>
<dd>Model comparison is predictive rather than a test, and it needs a
sampled fit.</dd>
</dl>
</div>
...
`tidy()` follows broom convention: one row per term, columns that do not
change name between engines.
...
`ranef()` gives the random-effect predictions -- REML's BLUPs, with
intervals.
...
`glance()` gives one row of fit-level information. On an INLA fit the
sampler-specific columns are `NA`, because a deterministic Laplace
approximation has no chains to report:
...
estimate. `fb_as_draws_simple()` returns the draws as a named list, and
any derived quantity is computed on them directly.
...
qualified or `library(loo)` is attached first.
...
error. A difference smaller than about twice its standard error is not
a distinction worth acting on.
...
something misleading -- `augment()` on an INLA fit refuses by name,
because there is no retained per-row model frame to append fitted values
to.
...
**Interval semantics.** `predict()` returns posterior expected-response
intervals, not posterior-predictive ones: they exclude residual
observation noise.

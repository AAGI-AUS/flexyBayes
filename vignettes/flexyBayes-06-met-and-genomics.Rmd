---
title: "Tutorial 06: Multi-environment trials and genomic selection"
date: "2026-08-26"
output:
  rmarkdown::html_vignette:
    ...
    toc_depth: 2
vignette: >
  %\VignetteIndexEntry{Tutorial 06: Multi-environment trials and genomic selection}
...
2. **Genomic selection.** Modern breeding programmes have genome-wide
   ...
   candidate from its markers is *genomic prediction*, and selecting
...
#>   Random   : ~gen + diag(county):gen 
#>   Residual : ~dsum(~units | county) 
#>   Model    : G: gen iid; at_simple; R: units sectioned by county 
#>   Family   : gaussian / identity 
#>   Representation: exact (model, not inference)
#>   Engine:         brms / Stan HMC
#>   N        : 1188 design rows, 1152 observed responses
#>   na_action: augment; 36 missing response(s); 0 cell(s) completed
...
separate, while a location where it is small is one where they do not, and
...
variances and the specific variances, and the package would reconstruct
the implied location-by-location covariance
...
![Mean yield per genotype at each of the nine yan.winterwheat environments, one line per genotype -- lines that cross indicate genotypes swap rank across environments (genotype-by-environment interaction), while parallel lines would indicate a stable ranking.](flexyBayes-06-met-and-genomics-figs/yw-interaction-plot-1.png)
...
GBLUP and pedigree BLUP are structurally identical, and only the
...
available, the genomic-pedigree blend (the $\mathbf{H}$ matrix of
...
# Realised relationship matrix, scaled to unit mean diagonal. Dividing
# by the marker count here -- rather than by VanRaden's (2008)
# allele-frequency-weighted 2 * sum(p * (1 - p)) -- is the Astle and
# Balding (2009) scaling. See the "Kinship matrix scaling" pitfall in
# Section 7 for what that choice does to sigma_g^2 and heritability.

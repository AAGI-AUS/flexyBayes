...
(`0.10.0`, on the 0.10.x line) and the latest CRAN release (when
present). Older released versions are not back-patched.
...
Reports outside the package – e.g. issues in upstream `INLA` or `brms` –
should be directed to those projects’ security channels.
